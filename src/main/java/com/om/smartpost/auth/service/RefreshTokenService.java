...
import com.om.smartpost.auth.repository.RefreshTokenRepository;
...
//    validate the refresh token expiration
    public RefreshToken verifyExpiration(RefreshToken token){
        if(token.getExpiryDate().isBefore(Instant.now())) {
            ...
            throw new ApiException(HttpStatus.UNAUTHORIZED, ErrorCodes.UNAUTHENTICATED.toString(), "Refresh token expired");
        }
        return token;
    }
...
//    find token stored.
    ...
    public AuthResponse refreshToken(String requestRefreshToken) {
        return refreshTokenRepository.findByToken(requestRefreshToken)
                ...
                .orElseThrow(() -> new ApiException(HttpStatus.UNAUTHORIZED, ErrorCodes.UNAUTHENTICATED.toString(), "Refresh token is invalid"));
    }
